...
and g' env = function (* 各命令の即値最適化 (caml2html: simm13_gprime) *)
  ...
  | Ld(x, V(y), i, p) when M.mem y env -> Ld(x, C(M.find y env), i, p)
  | St(x, y, V(z), i, p) when M.mem z env -> St(x, y, C(M.find z env), i, p)
  | LdDF(x, V(y), i, p) when M.mem y env -> LdDF(x, C(M.find y env), i, p)
  | StDF(x, y, V(z), i, p) when M.mem z env -> StDF(x, y, C(M.find z env), i, p)
